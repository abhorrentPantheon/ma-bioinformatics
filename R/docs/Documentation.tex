...
\usepackage[T1]{fontenc}	% hyphenation using accented symbols works properly
\usepackage[utf8]{inputenc}	% use UTF8 encoding
\usepackage{fullpage}		% don't use big margins
...
%\usepackage{setspace}		% allows changing linespacing
\usepackage{booktabs}		% good quality tables
%\usepackage{multirow}		% have tables with merged rows
%\usepackage{lscape}		% allow landscape-oriented pages
\usepackage[hang,small,it]{caption}
                                % change caption style
\usepackage[pdftex]{hyperref}	% allow links in pdf document
                                %    (i.e. to chapters/figures/tables/references)
...
\renewcommand\floatpagefraction{.9} % use 0.9x the page size for floats
