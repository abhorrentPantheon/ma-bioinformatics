...
#	           2: cor_variables.csv file in which correlation coefficeint between all the metabolites is calculated 
#				  and is ordered in decreasing value of correlation coefficient
...
output_1<-data.frame(Variable_1=rownames(cor_data)[lower_tri[,1]],Variable_2=colnames(cor_data)[lower_tri[,2]],cor_coefficient=cor_data[lower_tri])

#reorder the data according to the corelation coefficient in decreasing order
output<-data.frame(output_1[order(output_1$cor_coefficient,decreasing=TRUE),],row.names=NULL)
