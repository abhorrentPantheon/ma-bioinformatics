...
colnames(hca_data) <- if 
    (length(grep("^X[\\d]",colnames(hca_data),perl=TRUE)) != 0) # then
    {gsub("^X([\\d].*)","\\1",colnames(hca_data),perl=TRUE)} else
    {colnames(hca_data)}
